...
class Node:
    ...
    def __init__(self):
        ...
        self.cost_so_far = 0
        self.sum = self.distance_to_target + self.cost_so_far

    def __init__(self, row_val, column_val, parent, distance, cost_so_far):
...
def is_cut_base(obstacle_integer_coordinates, center, node_to_go):
    ...
    if abs(center.row_val - node_to_go.row_val) <= TOLERANCE:
        ...
        if obstacle_integer_coordinates[1] <= center.row_val <= obstacle_integer_coordinates[1] + 1:
...
def turn_point(node, turn_angle, parent, target):
    ...
    return Node(new_y, new_x, parent, eucledian_distance([new_x,new_y],[target.column_val,target.row_val]), parent.cost_so_far + 3.1)
...
def find_point_straight_on_circle(center):
    ...
    if center.column_val - center.parent.column_val == 0:
         ...
         if center.row_val - center.parent.row_val < 0:
             return Node(center.row_val - 3.1, center.column_val, center, 0, 0)

         elif center.row_val - center.parent.row_val > 0:
             return Node(center.row_val + 3.1, center.column_val, center, 0, 0)

         else:
             return Node(center.row_val, center.column_val, center, 0, 0)
        ...
        return Node(y_val, x_val, center, 0 ,0)
...
def find_points_on_circle(center, radius, discretization_sensivity, target):
    ...
    for k in range(discretization_sensivity):
        ...
        points.append(Node(y_axis, x_axis, center, eucledian_distance([x_axis,y_axis],[target.column_val,target.row_val]), 3.1))
...
def find_points_on_circle_piece(center, discretization_sensivity, target):
    ...
    for k in range(discretization_sensivity + 1):
        ...
        points.append(turn_point(point_straight, angle, center, target))

    return points


def constrained_a_star(launch_point_coordinate, target_coordinate, obstacles_coordinates, row_num, column_num,discretization_sensitivity ,searching_sensitivity):
    ...
    target_node = Node(target_coordinate[1], target_coordinate[0], None, 0, sys.maxsize )

    target_cell = [int(target_coordinate[0]), int(target_coordinate[1])]

    launch_node = Node(launch_point_coordinate[1], launch_point_coordinate[0], None, eucledian_distance(launch_point_coordinate,target_coordinate),0)

    if 3.1 <= launch_node.distance_to_target < 6.2 and not(is_cut(obstacle_cells,launch_node,target_node)):

        target_node.set_parent(launch_node)

        target_node.cost_so_far = eucledian_distance(launch_point_coordinate, target_coordinate)

        target_node.sum = target_node.cost_so_far

        return target_node, [], [], closed, noChild, yesChild

    nodes = find_points_on_circle(launch_node, 3.1, 12, target_node)
    ...
    for node in nodes:
        ...
        if 0 <= node.row_val < row_num and 0 <= node.column_val < column_num:
            ...
            if does_contain(node.row_val, node.column_val, closed) is None:
                ...
                if not (is_cut(obstacle_cells, launch_node, node)) and not([int(node.column_val), int(node.row_val)] in obstacle_cells): # correction should be done here

                    if [int(node.column_val), int(node.row_val)] == target_cell and launch_node.distance_to_target >= 3.1\
    ...
    while True:
        ...
        if 3.1 <= closest_node.distance_to_target < 6.2 \
                and can_hit_target(closest_node, straight_point, target_node, math.pi / 6) \
                and not (is_cut(obstacle_cells, closest_node, target_node)) and not(target_coordinate in obstacles_coordinates):

            if target_node.sum > eucledian_distance([closest_node.column_val, closest_node.row_val], target_coordinate) + closest_node.cost_so_far:
                target_node.set_parent(closest_node)
                target_node.cost_so_far = eucledian_distance([closest_node.column_val, closest_node.row_val], target_coordinate) + closest_node.cost_so_far
                target_node.sum = target_node.cost_so_far
            #return target_node, all_opened, on_circle, closed, noChild, yesChild

        nodelar = find_points_on_circle_piece(closest_node, discretization_sensitivity, target_node)
                    ...
                    if 0 <= node.row_val < row_num and 0 <= node.column_val < column_num:
                        ...
                        if not (is_cut(obstacle_cells, closest_node, node)) and not([int(node.column_val),int(node.row_val)] in obstacle_cells):
                            ...
                            if [int(node.column_val), int(node.row_val)] == target_cell \
                                    and closest_node.distance_to_target >= 3.1 \
                                    and can_hit_target(closest_node,straight_point,target_node,math.pi/6)\
    ...
    return target_node, all_opened, on_circle,closed,noChild,yesChild, target_node.cost_so_far
