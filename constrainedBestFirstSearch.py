...
class Node:
    ...
    def __init__(self, row_val, column_val, parent, distance, cost_so_far):
        ...
        self.distance_from_launch = cost_so_far
...
def turn_point(node, turn_angle, parent, target):
    ...
    return Node(new_y, new_x, parent, eucledian_distance([new_x,new_y],[target.column_val,target.row_val]), parent.distance_from_launch + 3.1)
...
def constrained_best_first_search(launch_point_coordinate, target_coordinate, obstacles_coordinates, row_num, column_num, discretization_sensitivity ,searching_sensitivity):
    ...
    target_node = Node(target_coordinate[1], target_coordinate[0], None, 0,0)
    ...
    if 3.1 <= launch_node.distance_to_target < 6.2 and not(is_cut(obstacle_cells,launch_node,target_node)):
        ...
        target_node.distance_from_launch = eucledian_distance(launch_point_coordinate, target_coordinate)

        #target_node.set_distance(eucledian_distance(launch_point_coordinate, target_coordinate))

        return target_node, [], [], closed, noChild, yesChild,target_node.distance_from_launch
            ...
            if does_contain(node.row_val, node.column_val, closed) is None:
                ...
                if not (is_cut(obstacle_cells, launch_node, node)) and not([int(node.column_val),int(node.row_val)] in obstacle_cells) :
                    ...
                    if [int(node.column_val), int(node.row_val)] == target_cell and launch_node.distance_to_target >= 3.1\
                        ...
                        return target_node, [], on_circle, closed, noChild, yesChild,target_node.distance_from_launch
    ...
    while not(len(open) == 0):
        ...
        if 3.1 <= closest_node.distance_to_target < 6.2 \
            ...
            target_node.distance_from_launch = eucledian_distance([closest_node.column_val, closest_node.row_val], target_coordinate) + closest_node.distance_from_launch
            return target_node, all_opened, on_circle, closed, noChild, yesChild,target_node.distance_from_launch
